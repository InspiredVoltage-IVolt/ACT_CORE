...
using PhysicsC = ACT.Core.Constants.Physics_Constants;
...
namespace ACT.Core
{
    public static partial class Physics
    {
        ...
        /// <exception cref="ArithmeticException">range exception</exception>
        ...
        public static double ibeta(double aa, double bb, double xx)
        {
            ...
            {
                t = pseries(a, b, x);
                if (t <= PhysicsC.MACHEP)
                {
                    t = 1.0 - PhysicsC.MACHEP;
                }
                else
                {
                    t = 1.0 - t;
            ...
            y = a * Math.Log(x);
            t = b * Math.Log(xc);
            if ((a + b) < PhysicsC.MAXGAM && Math.Abs(y) < PhysicsC.MAXLOG && Math.Abs(t) < PhysicsC.MAXLOG)
            {
                t = Math.Pow(xc, b);
                t *= Math.Pow(x, a);
                t /= a;
        ...
        private static double pseries(double a, double b, double x)
        {
            ...
            u = a * Math.Log(x);
            if ((a + b) < PhysicsC.MAXGAM && Math.Abs(u) < PhysicsC.MAXLOG)
            {
                t = gamma(a + b) / (gamma(a) * gamma(b));
                s = s * t * Math.Pow(x, a);
            }
