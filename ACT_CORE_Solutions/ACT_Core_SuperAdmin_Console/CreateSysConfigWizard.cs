...
namespace ACT.SuperAdmin.Console
{
    ...
    {
        public static ACT.Core.Structs.SystemSettingsData START()
        {
            ACT.Core.Structs.SystemSettingsData _tmpReturn = new Core.Structs.SystemSettingsData();
            ...
            _tmpReturn.Dependancies = new Core.Structs.Dependancies();
            ...
            while (SC.ReadKey().Key != ConsoleKey.Escape)
            {
                ...
                _tmpReturn.Dependancies.Dlls.Add(new Core.Structs.Dll() { Filename = _FullFileLocation, Name = _DLLName, Id = Guid.NewGuid().ToString() });
            ...
            _tmpReturn.Dependancies.Actlib = new List<Core.Structs.Actlib>();
            ...
            while (SC.ReadKey().Key != ConsoleKey.Escape)
            {
                ...
                _tmpReturn.Dependancies.Actlib.Add(new Core.Structs.Actlib() { Filename = _FullFileLocation, Name = _DLLName, Fileversion = _FileVersion, Updatesource = _UpdateSource, Id = Guid.NewGuid().ToString() });
                SC.Write("Quit? (Q or ESC)");
            }
            ...
            _tmpReturn.Dependancies.NugetPackages = new List<Core.Structs.NugetPackage>();
            ...
            while (SC.ReadKey().Key != ConsoleKey.Escape)
            {
                ...
                _tmpReturn.Dependancies.NugetPackages.Add(new Core.Structs.NugetPackage() { Name = _Name, Version = _Version, MinVersion = _MinVersion, PackageSource = _UpdateSource });
                SC.Write("Quit? (Q or ESC)");
            }
            ...
            _tmpReturn.Plugins = new List<Core.Structs.ConfigurationDataPlugin>();
            int _Order = 1;
            while (SC.ReadKey().Key != ConsoleKey.Escape)
            {
                var _quit = SC.ReadKey().Key;
                ...
                _tmpReturn.Plugins.Add(new Core.Structs.ConfigurationDataPlugin()
                {
